...
class Model {
  ...
  constructor(properties) {
    ...
    this.attributes = attributeBuilderFactory('attribute').call(this, { properties: properties.attributes });
    this.associations = attributeBuilderFactory('association').call(this, { properties: properties.associations });

    buildChanges.call(this);
  }

  associationArguments(...args) {
...
function attributeBuilderFactory(type) {
  ...
  return function({ properties = {} } = {}) {
    const definitions = this.constructor[pluralType]();
    const AttributeClass = this.constructor[`${type}Class`]();
    const attributes = {};
    ...
    keys.forEach((attributeName) => {
      ...
      const args = this[`${type}Arguments`]({
        definition: definition,
        value: attributeName in properties ? properties[attributeName] : definition.default,
      });

      definition.mixins = definition.mixins || [];
      const NewAttributeClass = definition.mixins.reduce((superClass, mixin) => mixin(superClass), AttributeClass);
      const attribute = new NewAttributeClass(args);
